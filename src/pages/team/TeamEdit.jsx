...
const TeamEdit = ({ handleEditDialogClose, item }) => {
  const [input, setInput] = useState({ name: '', title: '' })
  ...
  useEffect(() => {
    setInput({ name: item.name, title: item.title });
  }, [item])
  ...
  return (
    <Box>
      <Stack gap={2} py={2}>
        ...
        <TextField required onChange={handleInputChange} value={input.title} name='title' id="standard-basic" label="Title" variant="outlined" size='small' />
      </Stack>
      <DialogActions>
